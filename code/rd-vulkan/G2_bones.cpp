...
qboolean G2_Set_Bone_Angles_Index(
							...
							CGhoul2Info *ghlInfo,
#endif
							boneInfo_v &blist, const int index,
...
qboolean G2_Get_Bone_Anim(CGhoul2Info *ghlInfo, boneInfo_v &blist, const char *boneName, const int currentTime,
						  float *currentFrame, int *startFrame, int *endFrame, int *flags, float *retAnimSpeed
#ifndef RENDERER
						  , qhandle_t *modelList, int modelIndex
#endif
						  )
{
	model_t		*mod_a = (model_t *)ghlInfo->animModel;
...
#endif
#ifdef RENDERER
	// SP has no VM-callback trace path at all -- it exposes a direct
	// server-side trace instead (ri.SV_Trace), which already knows about
	// entities, unlike ri.CM_BoxTrace's static-world-only fallback below.
	// See code/rd-rend2/G2_bones.cpp's Rag_Trace for the same one-liner.
...
static inline void G2_RagGetPelvisLumbarOffsets(CGhoul2Info &ghoul2, CRagDollUpdateParams *params, vec3_t pos, vec3_t dir, vec3_t animPos, vec3_t animDir)
{
	...
	G2API_GiveMeVectorFromMatrix(G2VFM_ARG(final), ORIGIN, G2VFM_VEC(animPos));
	G2API_GiveMeVectorFromMatrix(G2VFM_ARG(final), POSITIVE_X, G2VFM_VEC(animDir));
	...
	G2API_GiveMeVectorFromMatrix(G2VFM_ARG(final), ORIGIN, G2VFM_VEC(pos));
	G2API_GiveMeVectorFromMatrix(G2VFM_ARG(final), POSITIVE_X, G2VFM_VEC(dir));
#else
	//We have the anim matrix pelvis pos now, so get the normal one as well
	...
	G2_RagGetWorldAnimMatrix(ghoul2, ghoul2.mBlist[boneIndex], params, final);
	G2API_GiveMeVectorFromMatrix(G2VFM_ARG(final), ORIGIN, lumbarPos);
...
{ //now returns true if any bone was in solid, otherwise false
	...
	for (i = 0; i < numRags; i++)
	{
			...
			broadsword_ragtobase->integer)
		{
			...
			G2API_GiveMeVectorFromMatrix(G2VFM_ARG(worldBaseMatrix), ORIGIN, basePos);
...
#ifdef RENDERER
	// SP's sharedSetBoneIKStateParams_t has no forceAnimOnBone -- never force.
...
{
#ifdef RENDERER
	model_t *mod_a = (model_t *)ghoul2->animModel;
	int index = G2_Find_Bone(mod_a, ghoul2->mBlist, boneName);
	if (index == -1 && bAddIfNotFound)
	{
		index = G2_Add_Bone(mod_a, ghoul2->mBlist, boneName);
	}
	return index;
#else
