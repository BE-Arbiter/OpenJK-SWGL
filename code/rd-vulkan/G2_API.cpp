...
qboolean G2API_AttachG2Model(CGhoul2Info *ghlInfo, CGhoul2Info *ghlInfoTo, int toBoltIndex, int toModel)
{
	...
	if (G2_SetupModelPointers(ghlInfo)&&G2_SetupModelPointers(ghlInfoTo))
	{
		...
		if ( ghlInfoTo->mBltlist.size() > toBoltIndex &&
...
#ifdef RENDERER
// SP-only export (see code/rd-rend2/G2_API.cpp's own G2API_DetachEnt) --
// MP tracks bolt attachment differently (via mModelBoltLink) and has no
// export table entry for this at all.
...
// it relies on the cached boltInfo_t::position that doesn't exist in this
// build's SP layout (see G2_SetUpBolts in tr_ghoul2.cpp).
#ifndef RENDERER
...
void G2API_CollisionDetectCache(CCollisionRecord *collRecMap, CGhoul2Info_v &ghoul2, const vec3_t angles, const vec3_t position,
			...
			while (i < ghoul2.size())
			{
				...
				*/
				...
				{ //reworked so we only alloc once!
					...
					g2.mTransformedVertsArray = (g2vert_int_t *)Z_Malloc(iSize, TAG_GHOUL2, qtrue);
				}
...
#ifdef _G2_GORE
		G2_TraceModels(ghoul2, transRayStart, transRayEnd, collRecMap, entNum, (EG2_Collision)traceFlags, useLod, fRadius,0,0,0,0,0,qfalse);
#else
		G2_TraceModels(ghoul2, transRayStart, transRayEnd, collRecMap, entNum, (EG2_Collision)traceFlags, useLod, fRadius);
#endif
		int i;
										  ...
										  int frameNumber, int entNum, vec3_t rayStart, vec3_t rayEnd, vec3_t scale, IHeapAllocator *G2VertSpace, EG2_Collision eG2TraceType, int useLod, float fRadius)
{
	...
	if (1)
	{
		G2API_CollisionDetectCache(collRecMap, ghoul2, angles, position, frameNumber, entNum,
			rayStart, rayEnd, scale, G2VertSpace, eG2TraceType, useLod, fRadius);
		return;
	}
	*/
...
void G2API_AddSkinGore(CGhoul2Info_v &ghoul2,SSkinGoreData &gore)
{
	...
	for(lod=lodbias;lod<maxLod;lod++)
	{
		...
		G2_TraceModels(ghoul2, transHitLocation, transRayDirection, 0, gore.entNum, G2_NOCOLLIDE,lod,0.0f,gore.SSize,gore.TSize,gore.theta,gore.shader,&gore,qtrue);
	}
}
#endif
