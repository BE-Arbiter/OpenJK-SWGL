...
public:
	...
	g2vert_int_t				*TransformedVertsArray;
	int					traceFlags;
	bool				hitOne;
	float				m_fRadius;
	...
	g2vert_int_t				*initTransformedVertsArray,
	int					inittraceFlags,
#ifdef _G2_GORE
	float				fRadius,
	float				initssize,
...
void R_TransformEachSurface( const mdxmSurface_t *surface, vec3_t scale, IHeapAllocator *G2VertSpace, g2vert_int_t *TransformedVertsArray,CBoneCache *boneCache)
{
	int				 j, k;
	mdxmVertex_t 	*v;
	...
	TransformedVertsArray[surface->thisSurfaceIndex] = (g2vert_int_t)TransformedVerts;
	if (!TransformedVerts)
	{
...
void G2_TransformSurfaces(int surfaceNum, surfaceInfo_v &rootSList,
					CBoneCache *boneCache, const model_t *currentModel, int lod, vec3_t scale, IHeapAllocator *G2VertSpace, g2vert_int_t *TransformedVertArray, bool secondTimeAround)
{
	int	i;
	assert(currentModel);
...
//		assert(G2_MODEL_OK(&g));
		...
		{ //do not stomp if we're using zone space
			g.mTransformedVertsArray = (g2vert_int_t*)G2VertSpace->MiniHeapAlloc(mdxm->numSurfaces * sizeof (size_t));
...
void *G2_FindSurface(const model_s *mod_t, int index, int lod)
{
	// damn include file dependancies
	const model_t	*mod = mod_t;
	mdxmHeader_t *mdxm = mod->data.glm->header;
...
#ifdef RENDERER
// SP has no raw-buffer network serialization for ghoul2 models -- it uses the
// engine's save-game system instead (ojk::SavedGameHelper / sg_export on each
// struct, already implemented in code/game/ghoul2_shared.h). Mirrors
// code/rd-vanilla/G2_misc.cpp's G2_SaveGhoul2Models exactly.
