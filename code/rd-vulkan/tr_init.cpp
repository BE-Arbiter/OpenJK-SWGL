...
#ifdef RENDERER
// referenced via "extern cvar_t *com_buildScript;" in rd-common/tr_font.cpp
...
#ifdef RENDERER
	// SP has no ri.CL_WriteAVIVideoFrame / AVI capture path at all (TakeVideoFrame
	// isn't part of refexport_t on SP either) -- this command is never queued,
	// so this is unreachable, but still needs to link.
...
Q_EXPORT refexport_t* QDECL GetRefAPI( int apiVersion, refimport_t *rimp ) {
	...
	// SaveJPGToBuffer/LoadJPGFromBuffer are #ifdef JK2_MODE fields in
	// refexport_t -- JK2_MODE is never defined in this codebase, so these
	// don't actually exist to assign (false positive from the rend2 field
	// diff -- rend2 defines JK2_MODE support code that's dead here too).
	...
	// AnyLanguage_ReadCharFromString2's only definition (tr_font.cpp) is
	// #ifdef JK2_MODE, never defined in this codebase -- rd-rend2 leaves
	// this field null too (its assignment is JK2_MODE-gated as well), so
	// this isn't actually a gap: nothing calls it when JK2_MODE is off.
	...
	// R_GetEntityToken's COM_Parse has no session of its own to pair with
	// SP's Begin/End-nesting COM_ParseExt, and nothing in SP calls this
	// (SV_GetEntityToken is used instead) -- left unwired, matching
	// code/rd-rend2/tr_init.cpp's commented-out "MP only" entry.
