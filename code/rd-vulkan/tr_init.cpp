...
void R_Register( void )
{
	...
	r_showGBuffer						= Cvar_Get("r_showGBuffer",						"0",						CVAR_CHEAT, "Show the G-buffer extraction pass full-screen instead of the scene: 1 = depth, 2 = normal, 3 = motion vectors. Requires r_depthPrepass 1");
	ri.Cvar_CheckRange(r_showGBuffer, 0, 5, qtrue);
	r_gtao								= Cvar_Get("r_gtao",							"0",						CVAR_ARCHIVE_ND | CVAR_LATCH, "Ground-truth ambient occlusion over the G-buffer. Requires r_depthPrepass 1");
	ri.Cvar_CheckRange(r_gtao, 0, 1, qtrue);
	r_gtaoRadius						= Cvar_Get("r_gtaoRadius",					"48",						CVAR_ARCHIVE_ND, "GTAO sampling radius, in world units");
	ri.Cvar_CheckRange(r_gtaoRadius, 1, 512, qfalse);
	r_gtaoIntensity						= Cvar_Get("r_gtaoIntensity",				"1.0",						CVAR_ARCHIVE_ND, "GTAO power curve; higher darkens");
	ri.Cvar_CheckRange(r_gtaoIntensity, 0.1, 8, qfalse);
	r_gtaoSlices						= Cvar_Get("r_gtaoSlices",					"3",						CVAR_ARCHIVE_ND, "GTAO direction slices per pixel");
	ri.Cvar_CheckRange(r_gtaoSlices, 1, 8, qtrue);
	r_gtaoSteps							= Cvar_Get("r_gtaoSteps",					"6",						CVAR_ARCHIVE_ND, "GTAO horizon search steps per slice direction");
	ri.Cvar_CheckRange(r_gtaoSteps, 1, 16, qtrue);
	r_contactShadows					= Cvar_Get("r_contactShadows",				"0",						CVAR_ARCHIVE_ND, "Screen-space contact shadows from the map sun. Shares the GTAO pass, so it requires r_gtao 1");
	ri.Cvar_CheckRange(r_contactShadows, 0, 1, qtrue);
	r_contactShadowLength				= Cvar_Get("r_contactShadowLength",			"32",						CVAR_ARCHIVE_ND, "Contact shadow ray length, in world units");
	ri.Cvar_CheckRange(r_contactShadowLength, 1, 512, qfalse);
	r_contactShadowThickness			= Cvar_Get("r_contactShadowThickness",		"12",						CVAR_ARCHIVE_ND, "How deep behind a surface a contact shadow hit still counts, in world units");
	ri.Cvar_CheckRange(r_contactShadowThickness, 1, 256, qfalse);
	r_contactShadowSteps				= Cvar_Get("r_contactShadowSteps",			"12",						CVAR_ARCHIVE_ND, "Contact shadow ray march steps");
