...
=================
*/
void CG_GetWorldModelName(char* initalModelName,char* weaponModel) {

	Q_strncpyz(weaponModel, initalModelName, sizeof(weaponModel));
...
void InitItemForWeapon(gitem_t* item, int weaponNum) {
	...
	char world_model[64];
	CG_GetWorldModelName(weaponData[weaponNum].weaponMdl, world_model);
	item->world_model = G_NewString(world_model);
	
	Com_Printf(S_COLOR_CYAN"initialized a dynamic weapon autogenerated item:\n");
	Com_Printf(S_COLOR_CYAN"\t - classname : %s\n",item->classname);
	Com_Printf(S_COLOR_CYAN"\t - gitag : %d\n",item->giTag);
	Com_Printf(S_COLOR_CYAN"\t - gitagName : %s\n",item->giTagName);
	Com_Printf(S_COLOR_CYAN"\t - gitype: %d\n",item->giType);
	Com_Printf(S_COLOR_CYAN"\t - icon: %s\n",item->icon);
	Com_Printf(S_COLOR_CYAN"\t - world_model: %s\n",item->world_model);
}
/*
=================
CG_RegisterWeapon
...
void CG_RegisterWeapon( int weaponNum ) {
	...
	{//in case the weaponmodel isn't _w, precache the _w.glm
		char weaponModel[64];
		CG_GetWorldModelName(currWeaponMdl, weaponModel);
