			...
			if ( hitEnt != NULL )
			{
				if ( hitEnt->client )
				{
					...
					if ( npc_class == CLASS_SEEKER || npc_class == CLASS_PROBE || npc_class == CLASS_MOUSE || npc_class == CLASS_REMOTE ||
					     ...
					     npc_class == CLASS_INTERROGATOR || npc_class == CLASS_ATST || npc_class == CLASS_SENTRY || GEntity_HasAttribute(hitEnt, ATTR_DROID))
					{
						if (!WP_SaberBladeUseSecondBladeStyle(saber, bladeNum)
							&& saber->hitOtherEffect)
						{
...
{//FIXME: probably need a debounce time
	...
	if (!noFail)
	{
		...
		if (thrown)
		{
			...
			if (self != player && GEntity_HasAttribute(self, ATTR_INQUISITOR))
			{
				...
				else
				{
					...
					if (pull)
					{
						...
						{//*SIGH*... if an NPC can't flee, they can't run after and pick up their weapon, do don't drop it
							...
							&& !(GEntity_HasAttribute(push_list[x], ATTR_HERO)) // Heroes can't be disarmed
							)
	...
	{//hit an organic non-player
		...
		{//an enemy
			...
			{//control them, even jedi
				...
				&& !(GEntity_HasAttribute(traceEnt, ATTR_HERO))
				&& !(GEntity_HasAttribute(traceEnt, ATTR_DROID))
				&& !(GEntity_HasAttribute(traceEnt, ATTR_COMMANDO)))
			...
			{
				if (!(GEntity_HasAttribute(traceEnt, ATTR_DROID)))
	...
	if (traceEnt->client)
	{
		...
		{//if we pick up & carry, drop their weap
			if (traceEnt->s.weapon
				...
				&& !FalseEmperorMission() // Player shouldn't be disarmed in the False Emperor mission (because that would be very bad)
				&& !(GEntity_HasAttribute(traceEnt, ATTR_HERO)) // Heroes can't be disarmed
				&& !(GEntity_HasAttribute(traceEnt, ATTR_COMMANDO)) // Commandos can't be disarmed either
				)
			{
...
void ForceShootLightning(gentity_t* self)
{
	...
	if (self->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2 && !(GEntity_HasAttribute(self, ATTR_PRECISE_LIGHTNING)))
	{//arc
...
qboolean FP_ForceDrainableEnt(gentity_t* victim)
{
	...
	if (GEntity_HasAttribute(victim, ATTR_DROID))
	{
		return qfalse;
	}
...
{//a jedi who broke free FIXME: maybe have some minimum grip length- a reaction time?
	...
	&& gripEnt->client
	&& (GEntity_HasAttribute(gripEnt, ATTR_COMMANDO))
		&& !Q_irand(0, 100 - (gripEnt->NPC->stats.evasion * 8) - (g_spskill->integer * 20))
		&& gripEnt->health > 0)
{
...
else if (GEntity_HasAttribute(gripEnt, ATTR_DROID )&& self->client->ps.forcePowerLevel[FP_GRIP] == FORCE_LEVEL_1)
...
else
{
	...
	if (gripEnt->client && gripEnt->health > 0)
	{
		...
		{//in air, set on whole body
			if (GEntity_HasAttribute(gripEnt, ATTR_DROID))
