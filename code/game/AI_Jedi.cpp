...
{
	if ( (NPC->client->NPC_class == CLASS_KYLE && (NPC->spawnflags&1)) || GEntity_HasAttribute(NPC, ATTR_BRAWLER))
	{//Boss Kyle
...
void NPC_BSJedi_FollowLeader(void)
{
			...
			|| !Q_stricmp(LORD_STK_TAT, NPC->NPC_type)))
	{
			...
			|| !Q_stricmp(VADER_INFINITIES, NPC->NPC_type))
		{
			...
			if ((GEntity_HasAttribute(NPC, ATTR_HELD_BY_HATRED) && NPC->max_health < 100) || (!(GEntity_HasAttribute(NPC, ATTR_HELD_BY_HATRED)) && NPC->health <= (NPC->max_health * .20f)))
			{
		...
		{
			if ((GEntity_HasAttribute(NPC, ATTR_HELD_BY_HATRED) && NPC->max_health < 100))
			{
...
void NPC_BSJedi_Default(void)
{
	...
	if (GEntity_HasAttribute(NPC, ATTR_INQUISITOR))
		{
			saber_colors_t currentColor = NPC->client->ps.saber[0].blade[0].color;
