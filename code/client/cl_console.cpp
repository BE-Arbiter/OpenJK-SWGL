...
void Con_DrawNotify (void)
{
	...
	if (re.Language_IsAsian())
	{
		fFontScale = con.charWidth * 10.0f /
			re.Font_StrLenPixels("aaaaaaaaaa", iFontIndex, 1.0f,0.0f);
	...
	for (i= con.current-NUM_CON_TIMES+1 ; i<=con.current ; i++)
	{
		...
		if (re.Language_IsAsian())
		{
			...
			// and print...
			//
			re.Font_DrawString(con.xadjust*(con.xadjust + (1*SMALLCHAR_WIDTH/*aesthetics*/)), con.yadjust*(v), sTemp, g_color_table[currentColor], iFontIndex, -1, fFontScale, 1.0f);
...
//	qhandle_t		conShader;
	...
	for (i=0 ; i<rows ; i++, y -= iPixelHeightToAdvance, row--)
	{
		...
		if (re.Language_IsAsian())
		{
			...
			// and print...
			//
			re.Font_DrawString(con.xadjust*(con.xadjust + (1*SMALLCHAR_WIDTH/*(aesthetics)*/)), con.yadjust*(y), sTemp, g_color_table[currentColor], iFontIndex, -1, fFontScale, 1.0f);
		}
		else
		{
			for (x = 0; x < con.linewidth + 1 ; x++) {
